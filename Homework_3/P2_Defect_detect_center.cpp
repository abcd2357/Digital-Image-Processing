...

{
    ...
    for (i=0;i<Height;i++) {
        for (j=0;j<Width;j++) {
            ImageOutput[i][j] = (ImageInput[i][j] < T) ? 255:0;
        }
    }

    if (!(file = fopen("GearTooth_01.raw","wb"))) {
        cout << "Cannot write file: " << "GearTooth_01.raw" << endl;
        exit(1);
    }
    ...
    while(1){
        ...
        if (First_match == 1) {
            ...
            if (out_num == 10) {
                ...
                out_num = 0;
                char filename[32];
                snprintf(filename, sizeof(char) * 32, "cup_shrink %i.raw", count);
            ...
            if (!(file = fopen("cup_shrink_final.raw","wb"))) {
                cout << "Cannot write file: " << "cup_shrink_final.raw" << endl;
                exit(1);
            }
            fwrite(ImageOutput, sizeof(unsigned char), Width*Height, file);
            fclose(file);
    ...
    for (i=5;i<Height-5;i++) {
        for (j=5;j<Width-5;j++) {
            if(ImageOutput[i][j] == 255){
                cout << "Center_X = " << j << "  Center_Y = " << i << endl;
                X = X+j;
                Y = Y+i;
            }
        }
    ...
    X = X/4.0+0.5;
    Y = Y/4.0+0.5;
    cout << "Gear_Center_X = " << X << "  Gear_Center_Y = " << Y << endl;

    for (i=0;i<250;i++){
        if(ImageInput[Y][X+i] == 0) {cout << "radius = " << i << " pixels." << endl;
        break;}
    }
