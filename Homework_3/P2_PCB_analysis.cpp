...

{
    ...
    for (i=0;i<Height;i++) {
        for (j=0;j<Width;j++) {
            ImageOutput[i][j] = (ImageInput[i+2][j+2] < T) ? 0:255;
        }
    }
    if (!(file = fopen("PCB_01.raw","wb"))) {
        cout << "Cannot write file: " << "PCB_01.raw" << endl;
        exit(1);
    }
    ...
    while(flag){
        ...
        if (out_num < 6) {
            ...
            out_num++;
            char filename[32];
            snprintf(filename, sizeof(char) * 32, "PCB_shrink %i.raw", count);
