...

{
    ...
    int shrink_Conditional_pattern_num = 46;
    ...
    const unsigned char shrink_Conditional_pattern[46][9] {
            // TK4
    ...
    while(1){
        ...
        if (First_match == 1) {
            ...
            if (out_num == 10) {
                ...
                out_num = 0;
                char filename[32];
                snprintf(filename, sizeof(char) * 32, "maze_thin %i.raw", count);
            ...
            if (!(file = fopen("maze_thin_final.raw","wb"))) {
                cout << "Cannot write file: " << "maze_thin_final.raw" << endl;
                exit(1);
            }
            fwrite(ImageOutput, sizeof(unsigned char), Width*Height, file);
            fclose(file);
